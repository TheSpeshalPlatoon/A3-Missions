...
[] spawn {  //-- Server
	...
	while {sleep 5; true} do {  //-- Win conditions
		...
		if (count (playableUnits select {!isObjectHidden _x && side group _x == east}) == 0) exitWith {[[], {"resistance" call BIS_fnc_endMission}] remoteExec ["spawn", 0]};  //-- All EAST dead, BLUFOR wins
