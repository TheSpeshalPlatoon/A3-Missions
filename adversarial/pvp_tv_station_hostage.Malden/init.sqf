...
[] spawn {  //-- Server
	...
	while {sleep 5; true} do {  //-- Win conditions
		if (count (playableUnits select {!isObjectHidden _x && side _x == west}) == 0) exitWith {[[], {"resistance" call BIS_fnc_endMission}] remoteExec ["spawn", 0]};  //-- All BLUFOR dead, RESISTANCE wins
 ...
 if (isServer) then {_random = selectRandom [24,6]; skipTime (random _random)};