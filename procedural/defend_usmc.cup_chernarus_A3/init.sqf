tsp_fnc_defend_select = {
	...
	tsp_arsenal_item setDamage 1; [_center] remoteExec ["tsp_fnc_defend", 2];
};

tsp_fnc_defend = {
	params ["_center", ["_rounds", 5], ["_round", 0]];
	while {sleep 33; _round <= _rounds && count (playableUnits select {alive _x && !isObjectHidden _x}) > 0} do {
		...
		{  //-- For each item in faction
			...
			for "_i" from 0 to (round (_minimum + (_increment * (_round - _required)))) do {  //-- Create x groups based on _base and _increment
				...
				{
					_entity = if (_x isKindOf "CAManBase") then {_group createUnit [_x, _position, [], 1, "NONE"]} else {createVehicle [_x, _position, [], 0, "NONE"]};
					_entity allowFleeing 0; {_entity disableAI _x} forEach ["AUTOCOMBAT","FSM","RADIOPROTOCOL","COVER","SUPPRESSION"];
					if !(_x isKindOf "CAManBase") then {_group createVehicleCrew _entity; _entity forceSpeed 4};
		...
		waitUntil {sleep 3; count (allUnits select {side group _x == tsp_defend_side}) < 3 || count (playableUnits select {alive _x && !isObjectHidden _x}) == 0};
...
tsp_defend_arti = [12, ["Smoke_120mm_AMOS_White", "Sh_82mm_AMOS", "Sh_82mm_AMOS"]];
...
waitUntil {player getVariable ["role", ""] != ""};
systemChat "Shift + click to select a position."; openMap [true, false]; 
onMapSingleClick {if (_shift) then {[_pos] spawn tsp_fnc_defend_select}};
waitUntil {!isNil "tsp_defend_start"}; player addItem "ACE_Fortify";