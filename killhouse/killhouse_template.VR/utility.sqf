...
tsp_fnc_lights = {  //-- Toggle lights on/off
	params ["_lights", ["_sleep", 1], ["_distance", 550], ["_pathOff", getMissionPath+"off.ogg"], ["_pathOn", getMissionPath+"on.ogg"]]; 
	tsp_fnc_lights_wait = true;
	{
		_x hideObjectGlobal !isObjectHidden _x;
		playSound3D [if (damage _x > 0.1) then {_pathOff} else {_pathOn}, _x, false, getPosASL _x, 5, random 2 max 0.5 min 2, _distance];		
