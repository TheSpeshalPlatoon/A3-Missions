...
tsp_fnc_killhouse_menu = {
	params ["_start", "_end", "_area", ["_display", findDisplay 46 createDisplay "RscDisplayEmpty"]]; tsp_killhouse_params = _this;
	...
	_box = _display ctrlCreate ["RscText", 9998]; _w = 0.4; _h = 0.6; _y = 0.05; _box ctrlSetPosition [(1.0-_w)*0.5,((1.0-_h)*0.5)-_y,_w,_h];
	...
	_civilians = _display ctrlCreate ["RscXSliderH", 9999]; _w = 0.38; _h = 0.045; _y = 0; _civilians ctrlSetPosition [(1.0-_w)*0.5,((1.0-_h)*0.5)-_y,_w,_h];
	...
	_hostages = _display ctrlCreate ["RscXSliderH", 9999]; _w = 0.38; _h = 0.045; _y = -0.1; _hostages ctrlSetPosition [(1.0-_w)*0.5,((1.0-_h)*0.5)-_y,_w,_h];
	...
	_locks = _display ctrlCreate ["RscXSliderH", 9999]; _w = 0.38; _h = 0.045; _y = -0.2; _locks ctrlSetPosition [(1.0-_w)*0.5,((1.0-_h)*0.5)-_y,_w,_h];
	...
	_button = _display ctrlCreate ["RscButtonMenu", 9998]; _w = 0.4; _h = 0.042; _y = - 0.275; _button ctrlSetPosition [(1.0-_w)*0.5,((1.0-_h)*0.5)-_y,_w,_h];
	...
	_button ctrlAddEventHandler ["buttonClick", {
		params ["_control"]; tsp_killhouse_params params ["_start", "_end", "_area"]; 
        _side = if (tsp_killhouse_theme in ["Tanoa", "Livonia"]) then {resistance} else {east};
		[[
            _start, _end, _area, 6, 18, 5, 0,  //-- Width, height, size, sleep
			["Bro_MSW_2m"], "Bro_MSW_2m_conc", 2, ["Bro_MSW_2m_d", "Bro_MSW_2m_d", "Bro_MSW_2m_de"], "Bro_MSW_2m_d_conc",  //-- Wall stuff
            0.2, 0.95, tsp_killhouse_locks,  //-- Openings, doors, locked
			...
			0.5, missionNameSpace getVariable ["furniture_"+tsp_killhouse_theme, []]                           //-- Furniture chance, type
