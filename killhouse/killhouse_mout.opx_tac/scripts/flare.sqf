...
["tsp_cba_flare_jitter", "SLIDER", "Jitter", "Flare movement in air.", "TSP Flare", [0, 100, 20]] call tsp_fnc_setting;
...
tsp_fnc_flare = {  //-- Should only be run on flare owner
	...
	[[_light, _fake, getArray(configFile >> "CfgAmmo" >> typeOf _flare >> "lightColor")], tsp_fnc_flare_client] remoteExec ["spawn", 0, true]; _flare attachTo [_fake, [0,0,0]];
	if ((getPos _fake)#2 < _altitudeStop) then {_altitudeStop = -1};  //-- Dont kill flare if shot at ground
	if (_sound) then {playSound3D [tsp_path + "data\pop.ogg", _fake, false, getPosASL _fake, 3, 1, 5000]};
	if (_sound) then {_fake spawn {while {sleep 10; alive _this} do {playSound3D [tsp_path + "data\burn.ogg", _this, false, getPosASL _this, 1, 1, 50]}}};
	waitUntil {sleep 0.1; _fake setVelocity [random _jitter, random _jitter, -_velocity]; !alive _fake || time > _timeStop || (getPos _fake)#2 < _altitudeStop};
