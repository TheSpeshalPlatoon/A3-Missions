...
[] spawn {while {sleep (tsp_param_weatherTime*60); isServer && tsp_param_weather} do {[] spawn tsp_fnc_environment_weather}};

(date call BIS_fnc_sunriseSunsetTime) params ["_sunrise", "_sunset"];
while {sleep 10; isServer} do {setTimeMultiplier (if (daytime < _sunrise || daytime > _sunset) then {tsp_param_time_night} else {tsp_param_time_day})};
