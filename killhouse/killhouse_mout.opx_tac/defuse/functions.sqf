...
tfb_fnc_defuse_bombdialog = {
	...
	{  // set up colors of wires
		if (_x != "BLANK") then {
			_imageTxt = "defuse\" + "wire_" + (toLower  _x) + ".paa";
...
tfb_fnc_defuse_cutwire = {
	...
	_imageTxt = "defuse\" + "wire_cut_" + (toLower _color) + ".paa"; ctrlSetText [_displayOrControl, _imageTxt];  // set new cut wire image	
...
tfb_fnc_defuse_detonate = {
	...
	closeDialog 2;
	{
		_force = (getPosWorld _bomb vectorFromTo getPosWorld _x) vectorMultiply 2000; _force set [2, 2000];
		[[_x, _force, [random 1, 0, 0]], dev_fnc_ragdoll] remoteExec ["spawn", _x];
	} forEach (allUnits select {_x distance _bomb < 3});
	{[[10, 1, 10]] remoteExec ["addCamShake", _x]} forEach (playableUnits select {_x distance _bomb < 10});
	...
	_bomb spawn {sleep 3; deleteVehicle _this};
};
