...
[
	...
	"Attack", getPos task_airfield_close, {true}, {"airfield_close" call tsp_fnc_sector_check && (count (allUnits select {_x distance task_airfield_close < 300 && side _x == east}) < 2)}
] spawn tsp_fnc_task;
[
...
[
	...
	"Attack", getPos task_dam, {true}, {"dam" call tsp_fnc_sector_check && (count (allUnits select {_x distance task_dam < 200 && side _x == east}) < 2)}
] spawn tsp_fnc_task;
[
	...
	"Attack", getPos task_radar_close, {true}, {"radar_close" call tsp_fnc_sector_check && (count (allUnits select {_x distance task_radar_close < 200 && side _x == east}) < 2)}
] spawn tsp_fnc_task;
[
	...
	"Attack", getPos task_military, {true}, {"military" call tsp_fnc_sector_check && (count (allUnits select {_x distance task_military < 200 && side _x == east}) < 2)}
] spawn tsp_fnc_task;