...
[
	...
	"Move", getPos wesstm, {true},  {"gasstation" call tsp_fnc_sector_check && (count (allUnits select {_x inArea trigger_wetsmister && side _x == east}) < 1)}
] spawn tsp_fnc_task;
[
	west, ["alderney", "primary"], "Advance to Alderney", "Advance And Report if <marker name='marker_12'>Alderney</marker> is secure.", 
	"Move", getPos alder, {true}, {"mansion_in" call tsp_fnc_sector_check && (count (allUnits select {_x inArea trigger_alderney && side _x == east}) < 1)}
] spawn tsp_fnc_task;
[
	west, ["schottler", "primary"], "Secure Schottler", "Advance And Secure <marker name='marker_14'>Schottler</marker>.", 
	"Attack", getPos scjte, {true}, {"jumo" call tsp_fnc_sector_check && (count (allUnits select {_x inArea trigger_schottler && side _x == east}) < 1)}
] spawn tsp_fnc_task;
[
...
[
	...
	"Attack", getPos steinwa, {true}, {"staging" call tsp_fnc_sector_check && (count (allUnits select {_x inArea trigger_steinway && side _x == east}) < 1)}
] spawn tsp_fnc_task;
[
