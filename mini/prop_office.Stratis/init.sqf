...
//-- DECLARATIONS
    ...
    prop_totalTime = prop_releaseTime + (["seekTime", -1] call BIS_fnc_getParamValue);
...
//-- INIT
    ...
    //-- SERVER
        if (isServer) then {
            ...
            prop_whitelist = [];
            {prop_whitelist pushBack (typeOf _x)} forEach ((getMissionLayerEntities "prop_propable")#0);
            publicVariable "prop_whitelist";
            ...
            if (true) exitWith {};
                while {sleep 1; true} do {
                    ...
                    if (time > prop_totalTime) exitWith {[[], {"PROP" call BIS_fnc_endMission}] remoteExec ["spawn", 0]};
                };
    ...
    [] spawn {
        ...
        //-- STUFF FOR EVERYONE
            ...
            [_bottomRight] spawn {params ["_bottomRight"]; while {sleep 0.1; alive player} do {_bottomRight ctrlSetText ("Health: " + str ((player getVariable ["health", 1])*100) + "%")}};                                                                                                                                                    //-- Allow 3rd person
        ...
        //-- TIME REMAINING
            while {(serverTimeOnJoining + time) < prop_totalTime} do {hintSilent parseText ("<t size='2'>" + ([(prop_totalTime - (serverTimeOnJoining + time)), "MM:SS"] call BIS_fnc_secondsToString) + "</t>"); sleep 1};
    };
